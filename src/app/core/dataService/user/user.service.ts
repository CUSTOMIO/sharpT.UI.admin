...
import {  UsersDetail, UserName, UserEnrollDate } from '../../model';
...
export class UserService {
  ...
  public getUserStandard(userId: number): Observable<string> {
    const url = `${environment. api_endpoint}/getUserStandard/${userId}`;
    return this.http.get<string>(url);
  }

  public getUserSubject(userId: number): Observable<string[]> {
